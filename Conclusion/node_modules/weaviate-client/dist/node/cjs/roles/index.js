...
const roles = (connection) => {
  return {
    ...
    userAssignments: (roleName) =>
      connection.get(`/authz/roles/${roleName}/user-assignments`, true).then(util_js_1.Map.assignedUsers),
    create: (roleName, permissions) => {
      const perms = permissions
        ? util_js_1.Map.flattenPermissions(permissions).flatMap(util_js_1.Map.permissionToWeaviate)
        : undefined;
      return connection
    ...
    addPermissions: (roleName, permissions) =>
      connection.postEmpty(`/authz/roles/${roleName}/add-permissions`, {
        permissions: util_js_1.Map.flattenPermissions(permissions).flatMap(
          util_js_1.Map.permissionToWeaviate
        ),
      }),
...
exports.permissions = {
  ...
  data: (args) => {
    var _a;
    const collections = Array.isArray(args.collection) ? args.collection : [args.collection];
    const tenants = Array.isArray(args.tenant)
      ? args.tenant
      : [(_a = args.tenant) !== null && _a !== void 0 ? _a : '*'];
